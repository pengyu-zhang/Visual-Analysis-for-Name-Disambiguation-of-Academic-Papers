...
The collaboration structure and the author-journal relationships mirror the
paper's figures; paper titles, years, keywords and citation counts are
synthesized deterministically so that the dataset tells the same story as the
...
# Author -> journals, mirroring the bipartite author-journal chart in the
# paper's figures.
AUTHOR_JOURNALS = {
    "Jinpei Cheng": [
        "MATEC Web of Conferences",
...
# Collaboration edges, mirroring the collaboration network in the paper's
# figures, plus a few edges connecting the sensors/space sub-team to the
# rest of the network.
EDGES = [
...
def main():
    ...
    # 3) Make sure every author-journal relationship in AUTHOR_JOURNALS
    #    is covered by at least one paper.
    covered = set()
    for paper in papers:
